...
// the graph, which is simply an MST if the graph is connected.
...
namespace prim {
  ...
  std::vector<edge<V, E>> mst(const graph<V, E> &g) {
    ...
    while (!open.empty()) {
      ...
      // vertices must be disconnected from the current MST(s), so
      // we're starting an MST of a new connected component of the
      // graph.
